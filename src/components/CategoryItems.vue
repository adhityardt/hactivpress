  ...
  <div>
    <div v-for="(article, index) in articles" :key="index" class="card" style="margin-bottom:5px;">
      <div class="card-title">
        ...
        <button class="btn btn-light"> #{{article.data.category}}</button>
      </div>
      <div class="card-body">
        <h6 style="text-align:left;">
          <img src="@/assets/glyphicons-4-user.png" >
          {{usersDb.filter(function(el){return el['.key'] == $route.params.authorid})[0].name}} posted :
        </h6>
...
export default {
  ...
  methods:{
    getArticle () {
      db.ref('/Authors/' + localStorage.getItem('userIdPicked')).on('value', (snapshot) => {
        let articleList = []
